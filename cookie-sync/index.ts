...
const envConfig = fs.existsSync(ENV_PATH) ? dotenv.parse(fs.readFileSync(ENV_PATH)) : {};

// Configuration
const REDENV_TOKEN = envConfig.REDENV_TOKEN || process.env.REDENV_TOKEN;
const REDENV_TOKEN_ID = envConfig.REDENV_TOKEN_ID || process.env.REDENV_TOKEN_ID;
const REDENV_UPSTASH_URL = envConfig.REDENV_UPSTASH_URL || process.env.REDENV_UPSTASH_URL;
const REDENV_UPSTASH_TOKEN = envConfig.REDENV_UPSTASH_TOKEN || process.env.REDENV_UPSTASH_TOKEN;
...
if (!REDENV_TOKEN || !REDENV_TOKEN_ID || !REDENV_UPSTASH_URL || !REDENV_UPSTASH_TOKEN) {
  console.error("❌ Missing required .env variables (REDENV_TOKEN, REDENV_TOKEN_ID, REDENV_UPSTASH_URL, REDENV_UPSTASH_TOKEN)");
  process.exit(1);
}
...
async function run() {
  ...
  if (!envConfig.BROWSER_EXECUTABLE && !envConfig.BROWSER_CHANNEL) {
    if (process.stdout.isTTY) {
      ...
      if (answer === '2') {
        ...
        const execPath = await rl.question("Enter absolute path to executable: ");
  ...
  if (envConfig.BROWSER_EXECUTABLE) {
    ...
    console.log(`Using custom browser executable: ${launchOptions.executablePath}`);
  ...
  if (!fbIsLoggedIn) {
    if (FB_EMAIL && FB_PASSWORD) {
      ...
      console.error("❌ You are not logged into Facebook and no credentials were provided in .env!");
      console.error("👉 Please provide FB_EMAIL and FB_PASSWORD, or run 'bun run login' manually.");
    }
  }
  ...
  if (!finalFbIsLoggedIn) {
    ...
    console.log("⏸️ Script paused. Please solve the challenge in the open browser window.");
  ...
  if (finalFbIsLoggedIn) {
    const fbCookieString = newFbCookies.map(c => `${c.name}=${c.value}`).join('; ');
    
    // Attempt to extract fb_dtsg (sometimes needed for scraping APIs)
    ...
    if (fbDtsg) console.log(`✅ Extracted FB_DTSG: ${fbDtsg.slice(0, 10)}...`);

    console.log("Uploading FB credentials to Redenv...");
    await redenv.set("FB_COOKIE", fbCookieString);
    if (fbDtsg) {
      await redenv.set("FB_DTSG_TOKEN", fbDtsg);
    }
  }
